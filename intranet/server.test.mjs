...
test('local Galaxy and MGM pages use host reachability, not public internet state', async () => {
  ...
  const cloudApi = localAsset('cloud-api.js', await readFile(new URL('../cloud-api.js', import.meta.url), 'utf8'));
  const packagedCloudApi = localAsset('cloud-api.js', cloudApi);
  assert.ok(INTRANET_VERSION.endsWith('0.2.11'));
  ...
  assert.doesNotMatch(cloudApi, /https:\/\/amrs-api\.example\.invalid/);
  assert.match(cloudApi, /const DEFAULT_CLOUDFLARE_BASE_URL = "";/);
  assert.match(packagedCloudApi, /function normalizeQuery\(query\)/);
});
