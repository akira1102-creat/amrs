...
writeFileSync(join(target, 'Import.cmd'), [
  ...
  'if "%~1"=="" goto selectFiles',
  'if not exist "%~f1" goto mappingMissing',
  '"%~dp0node.exe" "%~dp0intranet\\cli.mjs" import "%~dp0data" "%~f1"',
  'if errorlevel 1 goto importFailed',
  'goto importSuccess',
  ':selectFiles',
  'echo 即將逐一選擇 11 個 AMRS Excel 工作簿；按取消可安全退出。',
  '"%~dp0node.exe" "%~dp0intranet\\cli.mjs" import-selected "%~dp0data"',
  'set "IMPORT_RESULT=%ERRORLEVEL%"',
  'if "%IMPORT_RESULT%"=="2" goto importCancelled',
  'if not "%IMPORT_RESULT%"=="0" goto importFailed',
  'goto importSuccess',
  ':importSuccess',
  'echo.',
  ...
  'exit /b 2',
  ':mappingMissing',
  'echo 找不到指定檔案。直接雙擊本程式可逐一選取 Excel 檔案。',
  'pause',
  'exit /b 2',
  ':importFailed',
