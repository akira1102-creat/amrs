...
test('all six maintenance workbooks submit, read back, and deduplicate without external requests', async () => {
  ...
  try {
    ...
    for (const [index, company] of COMPANIES.entries()) {
      const serialNo = String(9900 + index);
      const record = { company, casino: 'Synthetic QA', date: '2026/09/14', model: 'SAE', serialNo, reason: 'PM', actionTaken: 'Preventive Maintenance', inspector: 'Synthetic QA', poNumber: 'QA-ONLY', submissionId: `synthetic-only-${company}` };
      const result = await repository.postAction({ action: 'submitRecords', records: [record] });
      assert.equal(result.inserted, 1, `${company} should accept a local maintenance record`);
      const page = await repository.getAction({ action: 'dashboard', company, serialNo, includeParts: '0', refresh: '1' });
      assert.ok(JSON.stringify(page).includes(serialNo), `${company} should read back its local maintenance record`);
      const repeated = await repository.postAction({ action: 'submitRecords', records: [record] });
      assert.equal(repeated.inserted, 0, `${company} should not append the same submission twice`);
