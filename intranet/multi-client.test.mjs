...
test('four local API clients preserve submissions, retries and shared reads across restart', async () => {
  ...
  let runtime = openRuntime(directory);
  const originalFetch = globalThis.fetch;
  globalThis.fetch = () => { throw new Error('External requests forbidden'); };
  try {
    const clients = await Promise.all(Array.from({ length: 4 }, async (_, index) => {
      ...
      return { headers: { authorization: `Bearer ${session.token}`, 'content-type': 'application/json' }, payload: {
    ...
    const submit = client => runtime.handle(new Request('http://localhost/api', { method: 'POST', headers: client.headers, body: JSON.stringify(client.payload) }));
    ...
    runtime.close(); runtime = openRuntime(directory);
    const rows = (await runtime.sheets.valuesGet({ spreadsheetId: 'SCL', range: 'Worksheet!A2:Z' })).values;
    assert.equal(rows.length, 4);
    for (let i = 0; i < 4; i++) assert.equal(rows.filter(row => row.includes(`990${i}`)).length, 1);
    for (const client of clients) {
      const response = await runtime.handle(new Request('http://localhost/api?action=dashboard&company=SCL&includeParts=0&refresh=1', { headers: client.headers }));
      assert.equal(response.status, 200);
      const body = await response.text();
      for (let i = 0; i < 4; i++) assert.ok(body.includes(`990${i}`));
    }
  } finally { globalThis.fetch = originalFetch; runtime.close(); rmSync(directory, { recursive: true, force: true }); }
});
