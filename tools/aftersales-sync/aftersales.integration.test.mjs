...
import { AfterSalesClient, planSync, runSync } from './bridge.mjs';
...
test('published aftersales accepts a synthetic maintenance record through its live API', { skip: !existsSync(executable) }, async () => {
  ...
  try {
    ...
    try {
      seed.exec(`
        UPDATE users SET Role = 1, Status = 1 WHERE Username = 'synthetic_sync_user';
