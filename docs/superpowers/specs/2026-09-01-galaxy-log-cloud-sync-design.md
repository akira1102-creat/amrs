...
- 有網絡時按「匯入雲端」直接重新讀取指定的 Google Sheet；現場完成取 Log 的變更則先留在本機，之後按「同步」寫回。
- 保留 Excel 匯出功能，取消 CSV 匯出，不影響 AMRS 其他公司及功能。
...
Worker 內部 task 會附帶狀態、來源欄組／列位置及更新時間等同步資料，但這些欄位只存在 API／本機狀態，不會新增可見欄位到客戶 Sheet。若指定 Sheet 已有三欄一組資料，雲端讀取及同步均直接使用該格式。
...
- 未同步的 mutation outbox（現場完成、改回未取）。
...
1. 使用者按「匯入雲端」，瀏覽器以 `galaxyLogOverview` 強制重新讀取指定 Google Sheet。
2. 最新雲端清單會與本機尚未同步的 outbox 合併，避免覆蓋現場完成記錄。
3. 離線或雲端暫時不可用時保留上次本機 snapshot，恢復網絡後再按「匯入雲端」重試。
